...
function httpPostJson(string $url, array $payload, array $headers = []): array
{
    ...
    if ($status >= 400) {
        throw new RuntimeException(
            'HTTP ' . $status . ' da Solarman: ' . json_encode($decoded, JSON_UNESCAPED_UNICODE)
        );
    }
...
function getSolarman(array $cfg): array
{
    ...
    return [
        ...
        'voltage' => toFloatOrNull($flat['AV1'] ?? null),
        'load' => toFloatOrNull($flat['E_Puse_t1'] ?? null),
        'grid' => toFloatOrNull($flat['PG_Pt1'] ?? null),
        'pv' => toFloatOrNull($flat['PVTP'] ?? null),
        'battery_soc' => toFloatOrNull($flat['B_left_cap1'] ?? null),
        'battery_power' => toFloatOrNull($flat['B_P1'] ?? null),
    ];
}

function computeFlow(array $sample): array
{
    $pv = max(0.0, (float)($sample['pv'] ?? 0));
    $load = max(0.0, (float)($sample['load'] ?? 0));
    $grid = (float)($sample['grid'] ?? 0);
    $batteryPower = (float)($sample['battery_power'] ?? 0);
...
try {
    ...
    respond([
        ...
        'summary' => [
            'voltage_ac' => $sample['voltage'],
            'power_pv_total' => $sample['pv'],
            'load_power' => $sample['load'],
            'grid_power' => $sample['grid'],
            'battery_soc' => $sample['battery_soc'],
            'battery_power' => $sample['battery_power'],
        ],
        'flow' => $flow,
